...
# --- OBTENER DATOS DE RED SIN MENÚ ---
def get_network_info(interface_name):
    """Obtiene IP y Subnet dado el nombre de una interfaz (ej: wlan0)"""
    try:
...
def main():
    # --- PARSEO DE ARGUMENTOS PARA MODO AUTOMÁTICO ---
    parser = argparse.ArgumentParser(description='Net-Audit Pro - Herramienta de Auditoría')
    parser.add_argument('-i', '--interface', help='Interfaz de red (ej: wlan0, eth0)')
    parser.add_argument('-m', '--mode', choices=['1', '2', '3'], help='Modo de escaneo (1=Sigilo, 2=Agresivo, 3=Ofensivo)')
    ...
    if args.interface and args.mode and args.name:
        ...
        if not mi_ip:
            console.print(f"[red]❌ Error: La interfaz {args.interface} no tiene IP o no existe.[/red]")
            sys.exit(1)
    ...
    # --- MODO INTERACTIVO (MENÚS) ---
        ...
        console.clear()
        mostrar_banner()
        # Mostrar resumen solo en modo interactivo
    ...
    # --- EJECUCIÓN (COMÚN PARA AMBOS MODOS) ---
    ...
    resultados = []
    mensaje_progreso = f"[cyan]Analizando {len(hosts)} dispositivos ({modo['nombre']})..."
    ...
    with Progress(SpinnerColumn(), TextColumn("{task.description}"), BarColumn(), MofNCompleteColumn(), TimeElapsedColumn(), console=console) as progress:
        task = progress.add_task(mensaje_progreso, total=len(hosts))
